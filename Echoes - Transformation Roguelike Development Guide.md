...
- **Body Parts**: Head, torso, arms, legs from different animal archetypes
- **Animal Types**: Wolf (predator focus), Bear (strength/endurance), Eagle (precision/reach), Deer (speed/agility)
- **Synergy Engine**: Compatible parts create bonuses, conflicting parts create penalties
- **Identity Coherence**: Stability metric affecting overall performance and unlocking advanced abilities
- **Evolution Paths**: Branching development based on transformation choices and play style

### Player Progression & Stats
- **Layered Stat System**: Base stats + level bonuses + equipment bonuses = final stats
- **Core Stats**: moveSpeed, maxHP, attack, defense, critChance
...
class Player {
  ...
  private recalculateStats() {
    this.actualStats.moveSpeed = this.baseStats.moveSpeed + 
...
- **Descriptive naming**: `playerHealth` not `hp`, `actualMoveSpeed` not `speed`
...
- **Final values**: `actualStats.moveSpeed` (calculated properties)
