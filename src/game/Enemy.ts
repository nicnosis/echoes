...
export class Enemy {
  ...
  private somaDropCount: number = 3; // How many soma this enemy drops when it dies
  ...
  render(renderer: Renderer) {
    ...
    const timeInRadians =
      (this.animationTime / this.animationPeriod) * 2 * Math.PI;
  ...
  private createDeathSparks() {
    ...
    for (let i = 0; i < sparkCount; i++) {
      const angle =
        (Math.PI * 2 * i) / sparkCount + (Math.random() - 0.5) * 0.5; // Even distribution with some randomness
