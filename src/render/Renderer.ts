export class Renderer {
    ...
    private loadBackgroundImage() {
        ...
        this.backgroundImage.src = '/bg/meadow.png'
    }
    ...
    // Draw ellipse with camera-aware scaling
    drawEllipse(x: number, y: number, radiusX: number, radiusY: number, color: string, lineWidth: number = 0, strokeOnly: boolean = false, blurRadius: number = 0) {
        if (this.cam) {
            const screen = this.worldToScreen(x, y, this.cam)
            const scaledRadiusX = radiusX * this.cam.zoom
            const scaledRadiusY = radiusY * this.cam.zoom
            ...
            this.ctx.beginPath()
            this.ctx.ellipse(screen.x, screen.y, scaledRadiusX, scaledRadiusY, 0, 0, Math.PI * 2)
    ...
    drawBackground() {
        // Game world dimensions from CLAUDE.md: 1200x900
        const gameWorldWidth = 1200
        const gameWorldHeight = 900
        
        if (this.backgroundImage && this.backgroundImage.complete) {
            // Draw meadow background image covering entire game world
            // Position at center of game world (600, 450) with full dimensions
            this.drawImage(this.backgroundImage, gameWorldWidth / 2, gameWorldHeight / 2, gameWorldWidth, gameWorldHeight)
        } else {
            // Fallback to fern green if image not loaded
            const fernGreen = '#77b273'
            this.drawRect(gameWorldWidth / 2, gameWorldHeight / 2, gameWorldWidth, gameWorldHeight, fernGreen)
        }
    }
